...
export default{
  ...
  beforeMount(){
    let val = getValueFromStore(this.optionKey,this.seriesIndex,this.componentType)
    if(val){
      if(typeof val == 'number'){
        if(this.ui.includes('number-px')){
          this.curUi = 'number-px'
        }
      }else if(typeof val == 'string'){
          if(val.endsWith('%')){
            if(this.ui.includes('number-%')) {
              this.curUi = 'number-%'
            }
          }else{
            if(this.ui.includes('select')) {
              this.curUi = 'select'
            }
          }
      }
  ...
  render(h){
    ...
    /*Number =  <number disabled={disabled}  value={getValueFromStore(this.optionKey,this.seriesIndex,this.componentType)} unit={'px'}
      ...
      Number =  <number disabled={disabled}  value={getValueFromStore(this.optionKey,this.seriesIndex,this.componentType)} unit={'px'}
                        onInput={value=>updateOption(this.optionKey,value,this.seriesIndex,this.componentType)} min={this.min} max={this.max} step={this.step}></number>
    Number_pc =  <number disabled={disabled}  value={getValueFromStore(this.optionKey,this.seriesIndex,this.componentType)} unit={'%'}
                           onInput={value=>updateOption(this.optionKey,value,this.seriesIndex,this.componentType)} ></number>
