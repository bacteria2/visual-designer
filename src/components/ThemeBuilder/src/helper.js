function getOptions(e) {
  function a(e) {
    for (let a, t = ["北京", "天津", "河北", "河北", "山西", "内蒙古", "辽宁", "吉林", "黑龙江", "上海", "江苏"], l = 2002; 2007 >= l; l++) {
      let r = 0
        , o = 0;
      a = e[l];
      for (let n = 0, i = a.length; i > n; n++)
        r = Math.max(r, a[n]),
          o += a[n],
          e[l][n] = {
            name: t[n],
  ...
  let t = e ? e.theme.seriesCnt : 4
    ...
    for (let o = 0; t > o; ++o) {
      for (let n = [], i = 0; l > i; ++i) {
        if ("scatter" === e)
          let m = [Math.floor((600 * Math.random() + 400) * (t - o) / t), Math.floor((600 * Math.random() + 400) * (t - o) / t)];
        else
          let m = Math.floor((600 * Math.random() + 400) * (t - o) / t);
        n.push(m)
      }
      "radar" === e && (n = [n]),
        a.push({
    ...
    , m = function(e) {
    for (let a = i(e), t = 0; t < a.length; ++t)
      a[t].areaStyle = {
        normal: {}
      },
        a[t].stack = "total";
  ...
  for (let b = [{
    ...
    series: [{
      ...
      data: function() {
        for (let e = [], a = 0; t > a; ++a) {
          for (let l = [], r = 0, o = 0, n = Math.ceil(5 * Math.random()); n > o; ++o) {
            for (let i = [], m = 0, s = 0, u = Math.ceil(3 * Math.random()); u > s; ++s) {
              let d = Math.ceil(100 * Math.random());
              i.push({
                value: d,
                name: a + 1 + "-" + (o + 1) + "-" + (s + 1),
