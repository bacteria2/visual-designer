<template>
  <div class="option-adjust full-height edit">
    <div v-if="!renderError">
      <mu-drawer :open="propertyDrawer" class="pc-drawer" @close="">
        <vertical-tab-panel :isIndicator="false" isSelectColor v-model="editConfig.active">
          ...
          <vertical-tab :title="seriesConfig.title" :name="seriesConfig.name" v-if="series.length > 0">
            <vertical-tab-panel v-model="seriesConfig.active"
                                content-classes="vertical-tab__content__no-padding property-box">
              <vertical-tab v-for="(seriesPage,pageIndex) in seriesConfig.pages" :title="seriesPage.title"
                            :name="seriesPage.name" :key="seriesPage.name">
      ...
      <data-config-panel :show="showDataConfig" @showDataSetConfig="dataSetDialog = true" :seriesType="seriesType"
                         v-if="canDataConfig"></data-config-panel>
      <mu-dialog :open="dataSetDialog" title="" dialogClass="widget-dataset-dialog"
                 bodyClass="widget-dataset-dialogBody" actionsContainerClass="widget-dataset-action-zone"
  ...
  export default {
    ...
    methods: {
      ...
      saveWidgetInstance(){
        ...
        saveWidgetInstance(this.edittingWidget).then((resp) => {
          if (resp.success) {
            that.loading = false;
