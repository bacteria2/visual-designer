            ...
            <div style={{ height: 'calc(100vh - 128px)', overflow: 'hidden',display:'flex',flexDirection: 'column' }}>
                <Card bordered={false} bodyStyle={{ padding: '10px 32px' }}>
                    <Form layout="inline">
                        ...
                        <StandardFormRow title="所属分类"
                                         style={{ paddingBottom: 10, marginBottom: 10, display: this.state.display }}>
                            <FormItem>
                        ...
                        renderItem={item => (item ? (
                                <List.Item key={item._id}>
                                    <Card hoverable
                                          ...
                                          actions={[<Tooltip title="设计"><Icon type="setting"
                                                                              onClick={() => this.props.history.push(`/prototype/designer/${item._id}`)}/></Tooltip>,
                                              <Tooltip title="编辑"><Icon type="edit"/></Tooltip>,
                                              <Popconfirm title="确认是否删除原型组件?" onConfirm={() => this.protoDelete(item)}
                                        ...
                                        <div style={{ padding: '8px 0 8px 30px' }}>
                                            <a style={{
                                                fontSize: 16,
                                                color: '#676767' }}>{item.name ? item.name : '未命名'}</a>
                                        </div>
                                        <div style={{
                                            height: 153,
                                            display: 'flex',
                                ...
                                <List.Item>
                                    <Link to={'/prototype/designer/_addNew_'}>
                                        <Card bodyStyle={{ padding: '0' }} hoverable className={styles.card} style={{
                                            display: 'flex',
                                            justifyContent: 'center',
                                            height: 250,
                                            alignItems: 'center' }}>
