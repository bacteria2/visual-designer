...
let winPath= function (path) {
  return path.replace(/\\/g, '/');
}
let error = null;

function getConfig() {
  const resolvedFilePath = paths.mockPath;
...
function applyMock(devServer) {
  ...
  try {
    ...
    const watcher = chokidar.watch(paths.mockPath, {
...
function realApplyMock(devServer) {
  ...
  watcher.on('change', (path) => {
    console.log(chalk.green('CHANGED'), path.replace(paths.appDirectory, './mock'));
    watcher.close();
